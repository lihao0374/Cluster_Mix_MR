...
def main_worker(gpu_rank):
    ...
    if FLAGS.moxing:
        import moxing as mox
    from utils import Log, save_ckpt, adjust_learning_rate
    ############################
    # Set Log File #
    if FLAGS.moxing:
    ...
    log.logger.info(
        'Initialize optimizer: {\'decay_method: %s, batch_size(per GPU):%-4d, init_lr: %-.3f, momentum: %-.3f, weight_decay: %-.5f, lr_sche: %s, total_epoch: %-3d, num_workers(per GPU): %d, world_size: %d, rank:%d\'}'
    ...
    for batch_idx, (images, target) in enumerate(train_loader):
        ...
        if batch_idx % FLAGS.report_freq == 0:
            log.logger.info('==> Iter[%3d][%4d/%4d] loss : %2.5f Acc : %s' %
                            (epoch, batch_idx, nbatch_per_epoch, loss_total,
                             [prec[i][0].item() for i in range(len(outputs))]))
