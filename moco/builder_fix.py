...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k1, im_k2, im_label, lam, gpu_rank, node_rank, ngpu_per_node, nrank_per_subg, groups):
        ...
        len_MR = len(im_q)  # length of Multiple Resolutions
        q_MR = [] # like q_224, q_160
        for i in range(len_MR):
            q, q_mlp = self.encoder_q(im_q[i])  # queries: NxC
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            # Group Shuffle for im_k1
            for i in range(len_MR):
                # shuffle for making use of BN
                im_k = im_k1[i]
                im_k, idx_unshuffle = self._group_batch_shuffle_ddp(im_k, gpu_rank, node_rank, ngpu_per_node,
                                                                     nrank_per_subg, groups)
                k, k_mlp = self.encoder_k(im_k)  # keys: NxC
                k = nn.functional.normalize(k, dim=1)
                # undo shuffle
                k = self._group_batch_unshuffle_ddp(k, idx_unshuffle, gpu_rank, node_rank, ngpu_per_node,
                                                      nrank_per_subg,
                                                      groups)
                k1_MR.append(k)
            
            # Group Shuffle for im_k2
            for i in range(len_MR):
                # shuffle for making use of BN
                im_k = im_k2[i]
                im_k, idx_unshuffle = self._group_batch_shuffle_ddp(im_k, gpu_rank, node_rank, ngpu_per_node,
        ...
        # dequeue and enqueue
        self._dequeue_and_enqueue(k1_MR)
